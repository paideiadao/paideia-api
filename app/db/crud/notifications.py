...
def get_notifications(db: Session, user_id: int, skip: int = 0, limit: int = 10):
    return db.query(Notification).filter(Notification.user_id == user_id).order_by(Notification.date.desc()).offset(skip).limit(limit).all()


def create_notification(db: Session, user_id: int, notification: CreateAndUpdateNotification):
...
def edit_notification(db: Session, id: int, user_id: int, notification: CreateAndUpdateNotification):
    ...
    if user_id != db_notification.user_id:
        return JSONResponse(status_code=status.HTTP_401_UNAUTHORIZED, content="user not authorized to edit")
